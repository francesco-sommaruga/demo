...
const config = {
  ...
  // ! the macro should be able to handle video playback controls on respective pages
  content: [
    {
      title: "Web App",
      ...
      mode: "Fullscreen",
      controls: "duplicate",
      // !  added getControls to handle video playback controls
...
async function processWidget(e) {
  ...
  switch (command) {
    case "selection":
      ...
      const selectedContent = config.content[option];
      openWebview(selectedContent);

      // ! Determine controls dynamically
      let controlsToUse = selectedContent.controls;
      if (selectedContent.getControls) {
        controlsToUse = selectedContent.getControls(selectedContent.url);
      }

      if (controlsToUse === "duplicate" || controlsToUse === "webview") return;
      createPanel(controlsToUse);
      break;
    case "close":
      closeWebview();
      createPanel();
  ...
  return xapi.Status.Peripherals.ConnectedDevice.get()
    .then((devices) => {
      ...
      if (controller.length == 0) {
        console.log(
          `No in controllers connected, this macro is designed to work with an in room touch controller`
        );
        return false;
      } else {
        return true;
    ...
    .catch((e) => {
      console.log(
        "No connected devices, this macro is designed to work with an in room touch controller"
      );
      return false;
    });
}
        ...
        rows = rows.concat(
          row(
            widget(
              "instructions",
              "Text",
              "Hold the button to accelarate up",
              "size=4;align=center"
            )
          )
        );
        ...
        rows = rows.concat(
          row(
            widget(
              "instructions",
              "Text",
              "Use the arrows to move and rotate the pieces",
              "size=4;align=center"
            )
          )
        );
        rows = rows.concat(
          row(widget("navigation", "DirectionalPad", "", "size=3"))
        );
        break;
      case "playcontrols":
        rows = rows.concat(
        ...
        rows = rows.concat(
          row(widget("playcontrols-slider", "Slider", "", "size=4"))
        );
        rows = rows.concat(
          row([
            widget(
              "playcontrols-playpause",
              "Button",
              "",
            ...
            widget(
              "playcontrols-fastback",
              "Button",
              "",
              "size=1;icon=fast_bw"
            ),
            widget(
              "playcontrols-fastforward",
              "Button",
              "",
              "size=1;icon=fast_fw"
            ),
